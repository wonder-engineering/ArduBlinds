...
#define MOTOR_POSITION_OPEN  0                   // at powerup,
                                                 // assumes we're here
#define ONE_HUNDREDTH_MOTOR_POSITION_CLOSED 300  // moves to here
                                                 // if too light
...
#define LIGHT_SENSOR_DARK_THRESHOLD    800  // less than this will
                                            // trigger motion
#define LIGHT_SENSOR_LIGHT_THRESHOLD   400  // more than this will
                                            // trigger motion
...
void loop() {
  ...
  if (upButtonState == 0 && downButtonState ==0) {
    Serial.println(F("Both buttons pressed. Setting shade "
                     "position to 'dark' position!"));
    shade_position = dark;
  } else if (upButtonState == 0) {
    Serial.println("Stepping from dark to light position. ('up', "
                   "'blinds closed')");
    while (digitalRead(UP_BUTTON_PIN) == 0)
      stepper.step(1);
  } else if (downButtonState == 0) {
    Serial.println(F("Stepping from light toward dark position."
                     " ('down', 'blinds open')"));
    Serial.println(F(" To calibrate the string position "
                     "to the 'blinds open' position,"));
