...
#define MOTOR_CYCLE_DELAY_MS           100    // Delay while holding button
#define MOVEMENT_TIMEOUT_MS            10000  // don't try moving for
                                              // longer than this
enum motor_state {
  motor_forward = 0,
  motor_reverse = 1,
...
#define DATA_COLLECTION_INTERVAL_MS      1200000  // 20 min allows
                                        // 1 full day of data collection
struct DataElement{
  uint32_t timestamp;
...
uint32_t now_time  = 0;
uint32_t last_time = 500000;  // Init value causes us to collect
...
void dump_eeprom() {
  ...
  for (int idx = 0; idx < EEPROM.length(); idx += sizeof(DataElement)) {
    ...
    Serial.print(data_element.element.timestamp); Serial.print(F(" , "));
...
void button_move(ShadeDirection dir) {
  ...
  Serial.print(F("Moving shades with button in direction: "));
  Serial.println(dir);
...
void setup() {
  ...
  if (COLLECT_SENSOR_DATA) {
    setup_header(); Serial.println(F("Waiting 30 seconds before "
                                     "erasing last data collection..."));
    delay(30000);
    zeroize_eeprom();
  } else {
...
void loop() {
  ...
  if (openedButtonState == 0) {
    ...
    if (bright_counter > LIGHT_SENSOR_WAIT_PERIOD &&
      ...
      bright_counter = 0;
      sensor_state = bright;
      loop_header(); Serial.print("LIGHT condition detected! "
                                  "Last sensor value: ");
    ...
    if (dark_counter > LIGHT_SENSOR_WAIT_PERIOD  &&
      ...
      dark_counter = 0;
      sensor_state = dark;
      loop_header(); Serial.print(F("DARK condition detected! "
                                    "Last sensor value: "));
      Serial.println(light_sensor_value);
    }
  ...
  if (delta_time > DATA_COLLECTION_INTERVAL_MS && COLLECT_SENSOR_DATA) {
    ...
    if (eeprom_index + sizeof(data_element) < EEPROM.length()) {
      for (int i = 0; i < sizeof(data_element); i++) {
        // todo: check for data overflow here
        // (for now, just stop and don't overwrite)
      ...
      loop_header(); Serial.println(F("EEPROM is full - skipping data"
                                      " collection for point:"));
    }
